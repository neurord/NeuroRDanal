        ...
        for imol,mol in enumerate(plotmol):
            ...
            if num_spines>1 and plottype==2:
                for spnum,sp in enumerate(stimspines):
                    ...
                    axis[imol].plot(dataset.time_set[param][mol][0:maxpoint],np.mean(dataset.means['spines'][param][mol][0:maxpoint],axis=0).T[spnum], label=plotlabel+sp.split('[')[-1][0:-1],color=new_col)
            elif plottype==3:
                ...
                #loop over data.trials and plot trials separately, as in line 154
                    for t in range(len(dataset.trials)):
                        ...
                        axis[imol].plot(dataset.time_set[param][mol][0:maxpoint],dataset.means['spines'][param][mol][0:maxpoint],label='trial'+str(t),color=mycolor)
                else:
                    for spnum,sp in enumerate(stimspines):
